...
// Componente para exibir doadores sem localização no mapa
function DoadoresSemLocalizacao({
  doadores,
  onVerDoacoes,
}: {
  doadores: DoadorComDoacoes[];
  onVerDoacoes: (doador: DoadorComDoacoes) => void;
}) {
    ...
    <div className="w-full max-w-5xl mx-auto mt-8">
      <div className="bg-yellow-50 border-l-4 border-yellow-400 p-4 rounded-lg shadow mb-4">
        <h2 className="text-lg font-bold text-yellow-700 mb-2 flex items-center gap-2">
          <svg className="w-5 h-5 text-yellow-500" fill="currentColor" viewBox="0 0 20 20"><path d="M9 2a1 1 0 012 0v7a1 1 0 01-2 0V2zm1 13a1.5 1.5 0 100 3 1.5 1.5 0 000-3z"/></svg>
          Doadores sem localização no mapa
        </h2>
        <ul className="divide-y divide-yellow-100">
          {doadores.map(doador => (
            <li key={doador.doadorId} className="py-3 flex flex-col md:flex-row md:items-center md:justify-between">
              <div>
                <div className="font-semibold text-base text-yellow-900">{doador.razaoSocial}</div>
        ...
        <div className="text-xs text-yellow-700 mt-2">
          Não foi possível localizar esses endereços no mapa, mas você pode visualizar as doações normalmente.
        </div>
      </div>
    </div>
  );
...
// Componente para exibir doações de doadores sem localização
function ListaDoacoesSemLocalizacao({
  doadores,
  onVerDoacoes,
}: {
  doadores: DoadorComDoacoes[];
  ...
  // Filtra doadores que realmente têm doações disponíveis
  const doadoresComDoacoes = doadores
    .filter((d) => d.doacoes && d.doacoes.length > 0);

  if (doadoresComDoacoes.length === 0) return null;
  ...
  return (
    <div className="w-full max-w-5xl mx-auto mt-8">
      <div className="bg-yellow-50 border-l-4 border-yellow-400 p-4 rounded-lg shadow mb-4">
        <h2 className="text-lg font-bold text-yellow-700 mb-2 flex items-center gap-2">
          <svg className="w-5 h-5 text-yellow-500" fill="currentColor" viewBox="0 0 20 20"><path d="M9 2a1 1 0 012 0v7a1 1 0 01-2 0V2zm1 13a1.5 1.5 0 100 3 1.5 1.5 0 000-3z"/></svg>
          Doações de doadores sem localização no mapa
        </h2>
        <ul className="divide-y divide-yellow-100">
          {doadoresComDoacoes.map((doador) => (
            <li key={doador.doadorId} className="py-3">
              <div className="font-semibold text-base text-yellow-900">{doador.razaoSocial}</div>
              <div className="text-xs text-gray-700 mb-1">
                <span className="font-semibold">Endereço:</span>{" "}
                {formatarEndereco(doador.enderecoObj)}
              ...
              <div className="mt-2 space-y-2">
                {doador.doacoes.filter((d) => d.disponivel).map((doacao) => (
                  <div
                    key={doacao.id}
        ...
        <div className="text-xs text-yellow-700 mt-2">
          Não foi possível localizar esses endereços no mapa, mas você pode visualizar e reservar as doações normalmente.
        </div>
      </div>
    </div>
  );
...
export default function BuscarDoacao() {
  ...
  // Função para buscar doações e doadores
  ...
  // Busca doações e doadores ao montar o componente
  useEffect(() => {
  ...
  const handleReservar = async (doacaoId: string) => {
    ...
    try {
      // Obtém o beneficiarioId do usuário autenticado
      const token = localStorage.getItem("token");
      const auth = getAuth();
      const currentUser = auth.currentUser;
      if (!token || !currentUser) throw new Error("Usuário não autenticado");

      // Atualiza a doação usando o método PATCH na rota correta
      await fetch(`${import.meta.env.VITE_API_URL}/doacoes/${doacaoId}`, {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
        ...
        body: JSON.stringify({
          disponivel: false,
          beneficiarioId: currentUser.uid,
          dataReserva: new Date().toISOString(),
        }),
      });

      // Atualiza o modal imediatamente junto ao toast
      if (modalDoador) {
        const resp = await fetch(
          `${import.meta.env.VITE_API_URL}/doacoes/doador/${modalDoador.doadorId}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
              "Content-Type": "application/json",
        ...
        let novasDoacoes: Doacao[] = [];
        if (resp.ok) {
          const todas = await resp.json();
          novasDoacoes = todas.filter((d: Doacao) => d.disponivel);
        }
        ...
        // Se não houver mais doações disponíveis, fecha o modal e recarrega o mapa após um timeout
        if (novasDoacoes.length === 0) {
          setTimeout(() => {
            setModalDoador(null);
            setTimeout(() => {
              fetchDoacoesEUsuario();
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <main className="flex-1 py-8 px-2">
        ...
        >
          {/* Mapa dos doadores com localização */}
          <MapContainer
            center={mapCenter}
            zoom={13}
        ...
        {/* Lista de doações de doadores sem localização */}
        {doadores.filter(d => d.lat === 0 && d.lng === 0 && d.doacoes.some(doa => doa.disponivel)).length > 0 && (
          <div className="w-full max-w-5xl mx-auto mt-8">
            <div className="bg-yellow-50 border-l-4 border-yellow-400 p-4 rounded-lg shadow mb-4">
              <h2 className="text-lg font-bold text-yellow-700 mb-2 flex items-center gap-2">
                <svg className="w-5 h-5 text-yellow-500" fill="currentColor" viewBox="0 0 20 20"><path d="M9 2a1 1 0 012 0v7a1 1 0 01-2 0V2zm1 13a1.5 1.5 0 100 3 1.5 1.5 0 000-3z"/></svg>
              ...
              <ul className="divide-y divide-yellow-100">
                {doadores
                  .filter(d => d.lat === 0 && d.lng === 0 && d.doacoes.some(doa => doa.disponivel))
                  .map(doador => (
                    <li key={doador.doadorId} className="py-3">
                      <div className="font-semibold text-base text-yellow-900">{doador.razaoSocial}</div>
                      <div className="text-xs text-gray-700 mb-1">
        ...
        <DoadoresSemLocalizacao
          doadores={doadores.filter(d => d.lat === 0 && d.lng === 0)}
          onVerDoacoes={setModalDoador}
        />
        <ListaDoacoesSemLocalizacao
          doadores={doadores.filter(d => d.lat === 0 && d.lng === 0)}
          onVerDoacoes={setModalDoador}
        />
      </main>
